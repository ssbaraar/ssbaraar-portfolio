    ...
    <div className="flex flex-1 items-center justify-center p-8">
      <div className="w-full max-w-xs">
        <div className="mb-6 flex justify-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-coral/10">
            <Lock className="h-8 w-8 text-coral" />
          </div>
        </div>
        <h3 className="mb-1 text-center font-display text-xl font-bold">Admin Login</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={loading}
            className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-foreground text-sm font-semibold text-background disabled:opacity-50"
          >
...
function BotTrapPanel() {
  ...
  const fetchAttempts = React.useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/bot-attempts?limit=100");
  ...
  const clearAttempts = async () => {
    ...
    await fetch("/api/admin/bot-attempts", { method: "DELETE" });
    setAttempts([]);
  };

  React.useEffect(() => { fetchAttempts(); }, [fetchAttempts]);

  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <div className="flex items-center justify-between gap-2 border-b border-border bg-secondary/30 p-3">
        <div className="text-xs text-muted-foreground">{attempts.length} captured</div>
...
function StatCard({ icon: Icon, label, value, accent }: {
  icon: typeof BarChart3; label: string; value: number; accent: string;
}) {
  return (
...
function OverviewPanel() {
  ...
  if (loading && !data) {
    return <div className="flex flex-1 items-center justify-center"><RefreshCw className="h-6 w-6 animate-spin text-muted-foreground" /></div>;
  }
  if (!data) {
    return <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">Failed to load analytics</div>;
  }
    ...
    <div className="pretty-scroll flex-1 overflow-y-auto p-3 sm:p-4">
      {data.storage && !data.storage.configured && (
        <div className="mb-4 flex items-start gap-2 rounded-xl border border-amber/40 bg-amber/5 p-3 text-xs">
          ...
          <div>
            <strong>Storage: file-based (dev only).</strong> Set up Vercel KV for cross-instance persistence.{" "}
            <a href={data.storage.helpUrl} target="_blank" rel="noopener noreferrer" className="underline">Docs ↗</a>
          </div>
        </div>
      )}
      ...
      <div className="mb-4 rounded-2xl border border-border bg-background/60 p-3">
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Last 14 days</h3>
        ...
        <div className="flex h-24 items-end gap-1">
          {data.viewsByDay.slice(-14).map((d, i) => (
            <div key={i} className="group relative flex-1">
              <div className="w-full rounded-t bg-primary/70 transition-all hover:bg-primary"
      ...
      <div className="mb-4 rounded-2xl border border-border bg-background/60 p-3">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">By hour (UTC)</h3>
        <div className="flex h-16 items-end gap-0.5">
          {data.viewsByHour.map((h) => (
            <div key={h.hour} className="group relative flex-1">
              <div className="w-full rounded-t bg-lavender/60 hover:bg-lavender"
      ...
      <div className="mb-4 rounded-2xl border border-border bg-background/60 p-3">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Top pages</h3>
        <ul className="space-y-1.5">
          {data.viewsByPath.slice(0, 5).map((p) => (
            <li key={p.path} className="flex items-center gap-2 text-xs">
              ...
              <div className="h-1.5 w-12 overflow-hidden rounded-full bg-secondary">
                <div className="h-full bg-primary" style={{ width: `${(p.views / data.viewsByPath[0].views) * 100}%` }} />
              </div>
            </li>
          ))}
        </ul>
      ...
      {data.topReferrers.length > 0 && (
        <div className="rounded-2xl border border-border bg-background/60 p-3">
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Top referrers</h3>
          <ul className="space-y-1.5">
...
function VisitorsPanel() {
  ...
  const fetch_ = React.useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/analytics?limit=500");
      if (res.ok) setData(await res.json());
    } catch {
      // ignore
    } finally {
  ...
  return (
    <div className="pretty-scroll flex-1 overflow-y-auto p-3">
      <div className="mb-3 flex items-center justify-between">
        <div className="text-xs text-muted-foreground">{data.recentViews.length} recent · auto-refresh 15s</div>
        <button onClick={fetch_} className="text-xs text-muted-foreground hover:text-foreground">
          <RefreshCw className={`h-3 w-3 ${loading ? "animate-spin" : ""}`} />
        </button>
      </div>
        ...
        {data.recentViews.map((v, i) => (
          <li key={i} className="rounded-xl border border-border bg-background/60 p-2.5 text-xs">
            ...
            <div className="mt-1 flex items-center gap-2 text-[10px] text-muted-foreground">
              ...
              {v.referrer ? (
                <span className="truncate"><span className="opacity-60">from</span> {v.referrer}</span>
              ) : (
...
function PostsPanel() {
  ...
  const fetch_ = React.useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/posts?internal=1");
  ...
  const handleSave = async () => {
    ...
    if (!title.trim() || !bodyText.trim()) { setError("Title and body are required."); return; }
    setSaving(true);
    try {
      const res = await fetch("/api/admin/posts", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ title, excerpt, category, emoji, coverImage, keywords, body: bodyText }),
      });
      const data = await res.json();
      if (!data.ok) {
  ...
  const handleDelete = async (slug: string, isStored: boolean) => {
    if (!isStored) { alert("Built-in posts can only be deleted by editing src/lib/blog-posts.ts"); return; }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="pretty-scroll flex-1 overflow-y-auto p-3">
        {error && <div className="mb-3 rounded-xl border border-coral/40 bg-coral/5 p-2.5 text-xs text-coral">{error}</div>}
        {success && (
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <select value={category} onChange={(e) => setCategory(e.target.value as BlogPost["category"])}
                      ...
                      <option>Tutorial</option><option>Comparison</option>
                      <option>Case Study</option><option>Opinion</option>
                    </select>
                  </div>
                  ...
                  <div>
                    <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
                      <ImageIcon className="mr-1 inline h-3 w-3" />Cover image URL (optional)
                    </label>
                  ...
                  <div>
                    <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
                      Body * (markdown-lite — ## h2, - bullet, 1. num, &gt; quote, ```code```, ![alt](url))
                    </label>
        ...
        {loading ? (
          ...
          <ul className="space-y-1.5">
            {posts.map((p) => (
              <li key={p.slug} className="flex items-start gap-2 rounded-xl border border-border bg-background/60 p-2.5">
                <span className="text-base">{p.emoji}</span>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-xs font-semibold">{p.title}</div>
                  <div className="mt-0.5 flex flex-wrap items-center gap-1 text-[10px] text-muted-foreground">
                    <span>{p.category}</span><span>·</span>
                    <span>{p.readingTime}</span><span>·</span>
                    <span className="font-mono-jb">{p.publishedAt}</span>
                    {p._stored && (<><span>·</span><span className="rounded bg-lime/20 px-1 font-bold uppercase text-foreground">stored</span></>)}
                  </div>
                </div>
...
export function AdminPanel() {
  ...
  React.useEffect(() => {
    ...
    if (new URL(window.location.href).searchParams.get("admin") === "1") setOpen(true);
  }, []);
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-border p-3 sm:p-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="font-display text-sm font-bold tracking-tight sm:text-base">Dashboard</h2>
                  <p className="text-[10px] text-muted-foreground sm:text-xs">Analytics · posts · bot-trap</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {loggedIn && (
                  <button onClick={handleLogout}
                    className="inline-flex h-8 items-center gap-1.5 rounded-full border border-border px-3 text-xs text-muted-foreground hover:text-foreground">
